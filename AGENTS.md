...
## Fonts
- Self-hosted via Astro's Fonts API (`fonts` in `astro.config.mjs`), provider `fontProviders.google()` with `options.experimental.glyphs`. Google is contacted at build time only: `.woff2` files are emitted under `dist/_astro/fonts/` and there is no Google request at runtime.
- The glyph set is derived at config load by `collectGlyphs`, which walks `src/**`, so new copy is included automatically. Fonts are subset to those codepoints and carry a matching `unicode-range`; a character outside it (e.g. typed into the contact form) renders with the metric-matched fallback (Arial/Georgia).
- `text=` subsetting strips Google's subset metadata, so `<Font preload>` selectors must filter by `style`/`weight` only — `subset: 'latin'` matches nothing and silently drops the preload.
- Measured with Lighthouse 13.4.1 mobile on the built output (Chrome without extensions): subsetting took the three above-the-fold fonts from 344.6 KB to 145.5 KB and the simulated LCP from 3.98 s to 2.48 s (Performance 85 → 97).
- Families: `Fraunces` (display, variable `wght` + real italic, SOFT/WONK axes declared in `variableAxis`) → `--font-fraunces`; `Inter` (body/labels, variable `wght` only) → `--font-inter`.
- Fraunces is declared twice (normal + italic) because `experimental.glyphs` is per family entry: `collectGlyphs` builds the italic subset only from files matching `/italic|oblique|<em\b|<i\b/`, which is where italic copy lives. Italic display text in a file with none of those markers would render with the Georgia fallback — add the marker (or the file) if that ever happens.
- **`opsz` is deliberately not requested**, although the previous Fontsource files carried it. Keeping it (Fraunces `9..144`, Inter `14..32`) cost ~130 KB on the mobile critical path for a subtle optical-size difference at display sizes. Don't add it back without re-measuring.
...
- `.filmstrip-viewport` also carries `content-visibility: auto` + `contain-intrinsic-size: auto <height>` (one height per breakpoint) so Chrome neither renders nor fetches its images while the strip is off-screen — otherwise its lazy images load early and land in Lantern's pessimistic LCP graph. Measured: −74 ms of simulated LCP. `filmstrip.ts` self-heals through its ResizeObserver when the strip first renders (verified: scroll lands on the band centre and images load as they approach).
- Fonts: only the upright Fraunces is preloaded (`preload={[{ style: 'normal' }]}`). Preloading the italic too added 146 KB to the critical path for one word of the hero title (it is ~57 KB with its own subset now, still not worth preloading).
